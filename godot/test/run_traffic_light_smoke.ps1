...
# OpenDRIVE traffic-light smoke test (OsiTrafficLightVisualizer build + state API).
# Exits non-zero on failure so it can gate CI.
#
# Usage (from anywhere):
#   pwsh godot/test/run_traffic_light_smoke.ps1 [-Godot <path-to-godot_console.exe>]
param(
...
Write-Host "==> running headless traffic-light smoke test"
& $Godot --headless --path $proj --script "res://test/traffic_light_smoke.gd"
$code = $LASTEXITCODE
if ($code -eq 0) { Write-Host "TRAFFIC LIGHT OK" } else { Write-Host "TRAFFIC LIGHT FAILED ($code)" }
exit $code
